...
public class NhaXuatBanlmpl implements NhaXuatBan{
    ...
    public void danhsachNHAXUATBAN(JComboBox model) {
        ...
        for (NHAXUATBAN nxb : result) {
		 ...
		 public void danhSachNhaCungCap(DefaultComboBoxModel<String> model) {
		        try {
		            for (NHAXUATBAN ncc : NHAXUATBANDAO.getInstance().selectAll()) {
		                model.addElement(ncc.getTENNHAXUATBAN());
		            }
		        } catch (Exception ex) {
		            ex.printStackTrace();
		        }
